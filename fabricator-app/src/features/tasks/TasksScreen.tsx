...
import { PanResponder, Pressable, StyleSheet, Switch, TextInput, useWindowDimensions, View } from 'react-native';
...
const save=()=>{if(!title.trim())return;store.addTask(title.trim(),system.trim()||'General',addToBuyList);setTitle('');setSystem('Today');setAddToBuyList(false);};
const fastAdd=(text:string)=>{store.addTask(text,system.trim()||'Today',false);setSystem('Today');};
...
<View style={styles.purchaseRow}><View><AppText style={styles.purchaseTitle}>Add to Buy List</AppText><AppText style={styles.purchaseCopy}>Creates a matching item in Parts.</AppText></View><Switch value={addToBuyList} onValueChange={setAddToBuyList} trackColor={{false:colors.graphite,true:colors.orange}} thumbColor={colors.white} /></View>
