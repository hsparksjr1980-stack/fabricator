...
const actionIcons:any={voice:'microphone-outline',task:'clipboard-check-outline',part:'tools',photo:'camera-outline',render:'cube-outline'};
...
const activityIcons:any={task:'clipboard-check-outline',part:'package-variant-closed',photo:'camera-outline'};
...
const taskActivity=projectTasks.map(task=>({
id:`task-${task.id}`,
kind:'task',
label:'Task',
title:task.title,
meta:`${task.system} • ${task.status}`,
date:task.updatedAt||task.createdAt||'',
route:'Tasks'
}));
const partActivity=store.parts.filter(part=>part.projectId===store.selectedProjectId).map(part=>({
id:`part-${part.id}`,
kind:'part',
label:'Part',
title:part.name,
meta:`${part.system} • ${part.status}`,
date:part.updatedAt||part.createdAt||'',
route:'Parts'
}));
const photoActivity=store.photos.filter(photo=>photo.projectId===store.selectedProjectId).map(photo=>({
id:`photo-${photo.id}`,
kind:'photo',
label:'Photo',
title:photo.caption,
...
return [...taskActivity,...partActivity,...photoActivity]
.filter(item=>item.date)
.sort((a,b)=>timestampValue(b.date)-timestampValue(a.date))
.slice(0,6);
},[projectTasks,store.parts,store.photos,store.selectedProjectId]);
...
const goAction=(type:string)=>{ if(type==='voice')navigation.navigate('Voice'); if(type==='task')navigation.navigate('Tasks'); if(type==='part')navigation.navigate('Parts'); if(type==='photo')navigation.navigate('Photos'); if(type==='render')navigation.navigate('Render'); };
if(!p){
...
if(widget.type==='recentActivity') return shell(widget,<View style={styles.activityList}>{recentActivity.length?recentActivity.map(item=><Pressable key={item.id} style={({pressed})=>[styles.activityRow,pressed&&styles.pressed]} onPress={()=>navigation.navigate(item.route)}><View style={styles.activityIcon}><MaterialCommunityIcons name={activityIcons[item.kind]} size={18} color={colors.orange}/></View><View style={{flex:1}}><View style={styles.activityTop}><Label>{item.label}</Label><AppText style={styles.activityDate}>{formatActivityTime(item.date)}</AppText></View><AppText style={styles.activityTitle}>{item.title}</AppText><AppText style={styles.activityMeta}>{item.meta}</AppText></View></Pressable>):<AppText>No task, part, or photo activity yet.</AppText>}</View>,()=>navigation.navigate('Log'),'Tap to open the full activity timeline.',true);
